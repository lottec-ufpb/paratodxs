...
\newacronym{lpc}{LPC}{-- lógica proposicional clássica}
...
name=sentença,
plural=sentenças,
description={-- uma frase declarativa completa, uma oração, que pode ser verdadeira ou falsa.}
}
...
\newglossaryentry{formula aberta}{
  name = fórmula aberta,
  plural = fórmulas abertas,
  description = {-- uma fórmula que contém variáveis que não estão sob o escopo de um quantificador, isto é, uma fórmula que contém \definepl{ocorrencia livre}.}
}

\newglossaryentry{formula fechada}{
 name = fórmula fechada,
  plural = fórmulas fechadas,
  description = {-- uma fórmula cujas variáveis, \textit{caso ocorram}, estão todas sob o escopo de um quantificador, isto é, uma fórmula que contém apenas \definepl{ocorrencia ligada}.}
}

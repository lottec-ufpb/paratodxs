...
e este outro também:
	\begin{earg}
		\item[] O telefone toca.
		\item[] Se o telefone toca, então Pedro atende a chamada.
		\item[\therefore] Pedro atende a chamada.
	\end{earg}
Ambos os argumentos são válidos e não é difícil notar que eles compartilham uma estrutura ou forma comum, em conformidade com a Seção \ref{ss:ValidadeFormal}, que pode ser expressada assim:
	\begin{earg}
		\item[] $A$
		\item[] Se $A$, então $C$
		\item[\therefore] $C$
...
A validade dos argumentos que acabamos de considerar não tem muito a ver com os significados das expressões em português como `Sheila está melancólica', `Pedro atende a chamada' ou 'Leonardo atuou em vários filmes'.
...
Essa linguagem será a \emph{lógica proposicional}, ou LPC.
...
Considere agora este outro argumento:
	...
	\end{earg}
Novamente, parece impossível que a premissa seja verdadeira e a conclusão falsa, pois nada pode ser todo verde e todo vermelho ao mesmo tempo.
Portanto, o argumento é válido.
...
\emph{Na melhor das hipóteses, a lógica proposicional (LPC) nos ajudará a entender argumentos válidos devido à sua forma.}
...
Nossa linguagem artificial, a LPC (lógica proposicional), leva a cabo essa ideia de forma absolutamente implacável.
...
Usaremos as letras maiúsculas do alfabeto como letras sentenciais da LPC.
...
Estamos apenas dizendo que, por enquanto, usaremos a letra sentencial `$A$'  da LPC para simbolizar a sentença em português `Chove lá fora', e a letra sentencial `$C$' para simbolizar a sentença `Sheila está melancólica'.
Mais tarde, quando estivermos lidando com sentenças diferentes ou argumentos diferentes, poderemos fornecer uma nova chave de simbolização, tal como:
	\begin{ekey}
		\item[A] O telefone toca
		\item[C] Pedro atende a chamada
	\end{ekey}
É importante entender que qualquer que seja a estrutura interna que uma sentença em português possa ter, essa estrutura é perdida quando a sentença é simbolizada por uma letra sentencial da LPC.
Do ponto de vista da LPC, uma letra sentencial é apenas uma letra.
Ela pode ser usada para criar sentenças mais complexas, mas não pode ser desmontada.

\chapter{Conectivos}
\label{s:TFLConnectives}

No capítulo anterior, propusemos simbolizar as sentenças básicas do português através das letras sentenciais da lógica proposicional (LPC).
Neste capítulo veremos como simbolizar as expressões `e', `ou', `não',\ldots\ 
que normalmente ligam, conectam as sentenças.
As suas simbolizações na LPC são, por esta razão, chamadas de \emph{conectivos}.
Usaremos os conectivos lógicos para criar sentenças complexas a partir de componentes atômicos. Existem cinco conectivos na LPC.
...
Portanto, podemos simbolizar essa sentença na LPC como:
$$\enot\enot S$$
...
Como `estar infeliz' não significa o mesmo que `não estar feliz', então, para simbolizar a sentença \ref{not7}, precisamos de uma nova letra sentencial da LPC.

As sentenças \ref{not6} e \ref{not7} acima são melhor simbolizadas na LPC como:
	\begin{ekey}
		\item[F] Glória está feliz.
...
Talvez elas afetem a ênfase de uma frase, mas a ênfase é algo que em geral não interfere na validade ou não de um argumento e não é considerada na LPC.
...
Podemos então usar a seguinte chave
	...
	\end{ekey}
e simbolizar \ref{and5} na LPC como
$$(B_1 \eand R)$$
...
Como já propusemos a chave de que $B_1$ simboliza ``Bárbara é saxofonista'', então completamos nossa chave de simbolização com mais uma letra sentencial:
	...
	\end{ekey}
Agora, finalmente, usamos a paráfrase acima para simbolizar a sentença \ref{and6} como a seguinte sentença da LPC:
$$(B_1 \eand \enot C_1)$$
Observe que perdemos todas as nuances da sentença original em português nessa simbolização.
Existe uma clara diferença de tom, ênfase, entre a sentença `Embora Bárbara seja saxofonista, ela não é canhota' (a sentença \ref{and6}) e `Bárbara é saxofonista e não é o caso de que Bárbara é canhota' (a paráfrase que fizemos para simbolizá-la na LPC).
A LPC não preserva (e não pode) preservar esse tipo de nuance.

A sentença \ref{and7} levanta questões semelhantes.
Existe um claro contraste entre o que é dito de Romeu com o que é dito de Bárbara, mas isso não é algo com o qual a LPC consiga lidar.
...
Usaremos um símbolo novo da LPC para simbolizar este `ou'.
...
Na lógica, adotou-se a convenção de que o símbolo `\eor' da LPC sempre simboliza um \emph{ou inclusivo}.
Abaixo veremos como simbolizar um ou exclusivo na LPC usando os símbolos que já vimos `$\eor$', `$\eand$' e `$\enot$'.
...
Para simbolizar isso na LPC precisamos da disjunção e da negação.
...
Como isso nega toda a disjunção, simbolizamos a sentença \ref{or4} como:\footnote{
	...
	Mais adiante veremos que estas duas simbolizações distintas da sentença \ref{or4} como `$\enot(S_1 \eor S_2)$' ou como `$(\enot S_1 \eand \enot S_2)$' são, de uma perspectiva lógica, perfeitamente equivalentes na LPC.}
$$\enot(S_1 \eor S_2)$$
...
Embora o símbolo `\eor' da LPC sempre simbolize o \emph{ou inclusivo}, o \emph{ou exclusivo} pode também ser simbolizado na LPC.
...
Portanto, como \ref{if1} é verdadeira e \ref{if2} é falsa, elas não dizem a mesma coisa e não podem, por isso, ser simbolizadas pela mesma sentença da  LPC.
...
De fato, o nosso uso dos condicionais na língua portuguesa é muito rico em informação e sutilezas que não cabem no conectivo `$\eif$' da LPC.
...
Portanto, assim como não precisamos de um novo símbolo da LPC para lidar com o \emph{ou exclusivo}, também não precisamos de um novo símbolo da LPC para lidar com os bicondicionais.
...
\section{A menos que}
Os cinco conectivos introduzidos até agora (`$\enot$', `$\eand$', `$\eor$', `$\eif$' e `$\eiff$') correspondem a todos os conectivos da LPC.
...
De fato, no Capítulo \ref{s:SemanticConcepts}, veremos que todas essas três simbolizações são sentenças equivalentes na LPC.
...
\practiceproblems
\solutions
\problempart Usando a chave a seguir, simbolize cada uma das 6 sentenças abaixo na LPC.\label{pr.monkeysuits}
	\begin{ekey}
...
\problempart Usando a chave a seguir, simbolize cada uma das 12 sentenças abaixo na LPC.
\begin{ekey}
...
\problempart Usando a chave a seguir, simbolize cada uma das 12 sentenças abaixo na LPC.\label{pr.avacareer}
	\begin{ekey}
...
\problempart
Usando a chave a seguir, simbolize cada uma das 9 sentenças abaixo na LPC.
...
\problempart
\label{pr.spies}
Proponha uma chave de simbolização e utilize-a para simbolizar  cada uma das 6 sentenças abaixo na LPC.
\begin{earg}
...
\solutions
\problempart Proponha uma chave de simbolização e utilize-a para simbolizar  cada uma das 5 sentenças abaixo na LPC.
\begin{earg}
...
\problempart
Para cada um dos 3 argumentos abaixo, identifique sua conclusão e suas premissas, proponha uma chave de simbolização e use-a para simbolizar todas as sentenças do argumento na LPC.
\begin{earg}
...
\problempart
Para cada um dos três argumentos abaixo, identifique sua conclusão e suas premissas, proponha uma chave de simbolização e utilize-a para simbolizar o  argumento da melhor maneira possível na LPC.
...
\chapter{Sentenças da LPC}\label{s:TFLSentences}
A expressão `as maçãs são vermelhas ou as jaboticabas são pretas' é uma sentença em português e a expressão `$(M \eor J)$' é uma sentença da LPC.
...
Neste capítulo, no entanto, ofereceremos uma \emph{definição} completa do que conta como uma sentença da LPC.
E esse é um dos motivos pelos quais dizemos que uma linguagem formal como a LPC é mais precisa do que uma linguagem natural como o português.


\section{Expressões}

Vimos que há três tipos diferentes de símbolos na LPC:
\begin{center}
\begin{tabular}{l l}
...
\end{tabular}
\end{center}
Definimos uma \xdefine{expressao da LPC} como qualquer sequência de símbolos da LPC.
Pegue uma quantidade qualquer de símbolos da LPC e escreva-os, em qualquer ordem, e você terá uma expressão da LPC.


\section{Sentenças}\label{s:Sentences}
Obviamente, muitas expressões de LPC serão totalmente sem sentido.
Nós precisamos saber quando uma expressão de LPC constitui uma \emph{sentença}.
...
Considere a negação: dada qualquer sentença \meta{A} da LPC, $\enot\meta{A}$ é uma sentença da LPC.
(Por que as fontes engraçadas? Trataremos disso na Seção \ref{s:Metavariables}.)

Podemos fazer o mesmo para cada um dos outros conectivos da LPC.
Por exemplo, se \meta{A} e \meta{B} são sentenças da LPC, então $(\meta{A}\eand\meta{B})$ é uma sentença da LPC.
...
Na definição de sentença da LPC acima, a cláusula básica é a 1; as cláusulas 2 a 6 correspondem às regras para a geração de novos elementos a partir dos já estabelecidos, e a cláusula 7 é o fechamento, que garante que nada além do que for assim gerado será uma sentença.
...
a regra para geração de novos elementos e o fechamento são dados pelas seguintes cláusulas:
	\begin{ebullet}
		\item Se $x$ é meu acestral, então os pais de $x$ são meus ancestrais.
...
O mesmo se aplica à nossa definição indutiva de sentenças de LPC.
...
Bem, `$D$' é uma letra sentencial da LPC, então, sabemos que `$D$' é uma sentença por causa da primeira cláusula da definição.
...
A estrutura indutiva das sentenças da LPC será importante quando considerarmos as circunstâncias sob as quais determinada sentença seria verdadeira ou falsa.
...
A estrutura indutiva das sentenças da LPC também nos permite dar uma definição formal da noção de \emph{escopo} de uma negação (mencionado na Seção  \ref{s:ConnectiveConjunction}).
...
É apenas uma sequência de símbolos da LPC, ou seja, uma \emph{expressão}.

No entanto, em muitas situações em que a LPC é utilizada, as pessoas são menos rigorosas sobre o uso dos parênteses.
...
\problempart
\label{pr.wiffTFL}
Para cada uma das $8$ expressões abaixo, decida (a) se ela é uma sentença da LPC, estritamente falando; (b) se é uma sentença da LPC quando permitimos as convenções sobre o uso de parênteses; (c) se ela for uma sentença apontada em (a) ou em (b), indique seu conectivo principal.
\begin{earg}
\item $(A)$
...
\problempart
Existem sentenças da LPC que não contêm letras sentenciais? Explique sua resposta.
...
Podemos usar a LPC para simbolizar ambas as leituras da passagem e, assim, fornecer uma desambiguação. Primeiro, precisamos de uma chave de simbolização:
\begin{ekey}
...
\begin{earg}
	\item Haskell caminhou até o banco e sentou-se.
...
Afinal, estamos descrevendo uma linguagem formal aqui, a LPC, e, portanto, muito frequentemente \emph{mencionaremos} as expressões da LPC.
...
A linguagem objeto que tem nos ocupado até aqui e também nas próximas Partes deste livro é a linguagem formal que estamos desenvolvendo: a LPC.
Já a metalinguagem, a linguagem que estamos usando para falar da LPC, é o português.
...
Por exemplo, temos usado as letras maiúsculas como letras sentenciais da LPC:
	$$A, B, C, Z, A_1, B_4, A_{25}, J_{375},\ldots$$
Esta é uma lista de sentenças da linguagem objeto (LPC).
As letras nesta lista não são sentenças do português.
Portanto, não devemos dizer, por exemplo:
	\begin{ebullet}
		\item $D$ é uma letra sentencial da LPC.
	\end{ebullet}
Estamos, obviamente, tentando criar uma sentença em português que diga algo sobre a linguagem objeto (LPC), mas `$D$' é uma sentença da LPC, e não parte do português.
...
Da mesma forma, o que pretendíamos dizer acima era apenas:
	\begin{ebullet}
		\item `$D$' é uma letra sentencial da LPC.
	\end{ebullet}
O ponto geral é que, sempre que quisermos falar em português sobre alguma expressão específica da LPC, precisamos indicar que estamos \emph{mencionando} a expressão, em vez de \emph{usando}-a.
...
Estas duas maneiras de apresentar indicam que a sentença da LPC está sendo mencionada, não usada.
Estamos, nos dois casos, apenas nos referindo, através da nossa metalinguagem (o português aumentado), a uma expressão da LPC.


\section{Metavariáveis}\label{s:Metavariables}
Nós, no entanto, não queremos falar apenas sobre expressões \emph{específicas} da LPC. 
Queremos também poder falar sobre \emph{qualquer sentença arbitrária} da LPC.
Nós, de fato, já fizemos isso na Seção \ref{s:Sentences}, quando apresentamos a definição indutiva de sentença da LPC.
Usamos as letras maiúsculas cursivas para fazer isso, a saber:
	$$\meta{A}, \meta{B}, \meta{C}, \meta{D}, \ldots$$
Diferentemente das letras sentenciais, essas letras não pertencem à LPC.
Elas, na verdade, fazem parte da metalinguagem (o português aumentado) que usamos para falar sobre as expressões da LPC.
Neste caso, usamos as letras maiúsculas cursivas para nos referir genericamente a uma expressão \emph{qualquer} da LPC.
Por exemplo, a segunda cláusula da definição indutiva de sentença da LPC foi assim expressada:
	\begin{earg}
		\item[2.] Se $\meta{A}$ é uma sentença, então $\enot \meta{A}$ é uma sentença.
	\end{earg}
Nesta cláusula, `$\meta{A}$' se refere de modo genérico a uma expressão qualquer da LPC.
Ela não é uma expressão específica da LPC, como `$A$', mas uma váriável da metalinguagem (metavariável) que se refere genericamente a qualquer expressão da LPC, tal como $$A,\ \ B,\ \  (A \eif (Q \eand R)),\ \  \enot\enot(B_3 \eif Z),\ \ldots$$
...
Para enfatizar:
	\factoidbox{
`$\meta{A}$' é um símbolo, que chamamos de \define{metavariables} e que usamos para falar sobre qualquer expressão da LPC.}

`$A$' é apenas uma letra sentencial específica da LPC.
Nós incluímos as metavariáveis `$\meta{A}$', `$\meta{B}$', `$\meta{C}$',\ldots\ ao português, para construir nossa metalinguagem, o \emph{português aumentado}, que usamos para falar da LPC.
...
`$\enot\meta{A}$' não é uma sentença da LPC, pois `$\meta{A}$' é um símbolo da metalinguagem, do português aumentado, e não um símbolo da LPC.
...
Um dos nossos principais objetivos para o uso da LPC é estudar argumentos. Este será o foco das Partes \ref{ch.TruthTables} e \ref{ch.NDTFL} do livro.
...
Da mesma forma que podemos simbolizar sentenças do português na LPC, podemos também simbolizar argumentos em português usando a LPC.
Assim, poderíamos, por exemplo, perguntar se o argumento cujas premissas são as sentenças da LPC `$A$' e `$A \eif C$' e cuja conclusão é `$C$' é válido.
...
(Note que `$\therefore$' é um símbolo de nossa \emph{metalinguagem}, o português aumentado, e não um novo símbolo de LPC.

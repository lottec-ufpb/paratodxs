...
\paragraph{Disjunção.} Lembre-se de que `$\eor$' sempre representa o `ou' inclusivo (Seção \ref{s:ConnectiveDisjunction}).
...
Por que a tabela é desse jeito? Ela sempre tem as mesmas sequências de V’s e F’s? Porque, seguindo esse esquema, garantimos que todas as combinações de V e F sejam contempladas.
...
Com base no que já sabemos sobre `$\entails$' e `$\eif$' podemos fazer as seguintes observações.
	...
	\end{ebullet}
Dado que uma sentença condicional é falsa apenas quando seu antecedente é verdadeiro e seu consequente é falso, podemos reescrever a observação 2.

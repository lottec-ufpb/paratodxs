...
\begin{earg}
	...
	\end{earg}
Repare que estamos \emph{temporariamente} desrespeitando a gramática da LPC aqui. 
...
As sentenças na forma normal disjuntiva satisfazem a seguinte propriedade:
	\factoidbox{\label{thm:dnf}\textbf{Teorema da Formal Normal Disjuntiva.} Dada qualquer sentença da LPC, existe uma sentença logicamente equivalente a ela que está na forma normal disjuntiva.
	}
...
Para obter uma sentença na FND que seja logicamente equivalente a $\meta{S}$, há dois casos a tratar:
	\begin{enumerate}
		...
		$$(\pm\meta{A}_{1} \land \ldots \land \pm\meta{A}_{n})$$
			onde cada literal $\pm\meta{A}_{j}$ ($1 \leq j \leq n$) será da forma $\meta{A}_{j}$ ou $\enot\meta{A}_{j}$ conforme as seguinte regras:
			\begin{align*}
...
Podemos agora propor e provar outro teorema sobre forma normal:
	\factoidbox{\label{thm:cnf}\textbf{Teorma da Forma Normal Conjuntiva.} Dada qualquer sentença da LPC, existe uma sentença logicamente equivalente a ela que está na forma normal conjuntiva.}

Seja $\meta{S}$ uma sentença LPC.
	\begin{itemize}
...
\section{Adequação Expressiva da LPC}  

Dos conectivos da LPC, a negação, `$\enot$', é um conectivo de um lugar (unário), ou seja, liga-se a uma única sentença, e todos os outros são conectivos de dois lugares (binários) que ligam-se a exatamente duas sentenças.
...
Será que para fazer isso nós precisamos adicionar à LPC o conectivo  `$\heartsuit$' e sua tabela característica?
Ou será que para dizer o que `$\heartsuit(A,B,C)$' diz, os conectivos que \emph{já existem} na LPC bastam?
...
Então, o modo mais geral e preciso da pergunta feita acima é: será que o conjunto total de conectivos da LPC, $\{\enot, \eand, \eor, \eif, \eiff\}$, é expressivamente adequado?
	\factoidbox{\label{thm:ExpressiveAdequacy}\textbf{Teorema da Adequação Expressiva.}
O conjunto dos conectivos da LPC é expressivamente adequado.
...
Dada qualquer tabela verdade, podemos usar o método empregado na prova do Teorema FND para obter uma sentença com os conectivos tradicionais da LPC cuja tabela de verdade é idêntica a esta dada.
...
Então é claro que o conjunto de todos os conectivos da LPC, que contém $\{\enot, \eand, \eor \}$, também é expressivamente adequado.
...
Em resumo, não há qualquer \emph{necessidade} de adicionar novos conectivos à LPC.
...
Eles não são incluídos nas apresentações típicas da LPC porque eles tornam as tabelas e as simbolizações das sentenças em português mais complicadas.
Mas a sua existência mostra que, se quiséssemos, poderíamos ter definido a linguagem da LPC com apenas um conectivo e, mesmo assim, ela seria expressivamente adequada.
...
Neste capítulo vamos relacionar os dois aspectos distintos nos quais estudamos a LPC: a sua \textit{semântica}, dada pelas tabelas de verdade (estudadas  na Parte~\ref{ch.TruthTables}), e o seu \emph{sistema formal}, dado pelo sistema de dedução natural (apresentado na Parte~\ref {ch.NDTFL}).
Nós vamos comprovar que o sistema formal da LPC é \textit{correto}: através dele só conseguimos provar formalmente a validade de argumentos que são de fato válidos quando analisados via tabelas de verdade.
...
O objetivo deste capítulo é provar que o sistema formal de provas da LPC apresentado na Parte~\ref {ch.NDTFL} é correto.
...
Usando as notações de metalinguagem que acabamos de relembrar, podemos dizer, então, que provar que o sistema formal da LPC é correto equivale a provar o seguinte teorema:
...
verificaremos individualmente cada uma das regras do sistema formal da LPC e mostraremos que nenhuma aplicação dessas regras nos leva ao erro.
...
O que nós queremos mostrar é que não é por mera coinciência que todas as linhas desta prova são inocentes.
	\begin{factoidboxe}\textbf{Lema da Inocência.}
		Todas as linhas de qualquer prova formal da LPC são inocentes.
	\end{factoidboxe}\noindent
Se todas as linhas de todas as provas que podem ser feitas na LPC são inocentes, então nós podemos confiar que as provas nunca vão nos desviar do caminho e nos levar ao erro.
De fato, é bem simples provar o Teorema da Correção quando assumimos como suposição que o Lema da Inocência vale.

\emph{Prova do Teorema da Correção}. Suponha que $\Gamma \proves \meta{C}$. Então, há uma prova na LPC cuja sentença da última linha é $\meta{C}$ e cujas suposições não descartadas estão todas entre as sentenças de $\Gamma$.
...
Para fazer isso começamos observando que cada linha de qualquer prova da LPC ou é uma suposição, ou é obtida aplicando-se alguma regra.
...
Dada uma prova qualquer da LPC e uma aplicação de regra qualquer (digamos que a linha $n$ de uma prova contém a sentença $\meta{A}$ obtida pela aplicação de uma regra qualquer G)
		   \begin{proof}
			   \have[\vdots]{ab}{\ \vdots}
...
O que nós queremos é mostrar que \emph{todas} as regras do sistema formal da LPC são regras confiáveis.
Antes de fazer isso, vejamos como a prova do Lema da Inocência fica simples quando assumimos que todas as regras de inferência da LPC são confiáveis.
...
\begin{itemize}
	...
	\item Mas e as provas de $3$ linhas? O caso é idêntico ao das provas de $2$ linhas. Ou a $3$ª linha é uma suposição ou foi obtida através da aplicação de uma regra de inferência.
		...
		\end{itemize}
	\item Bem, acho que você já pegou o espírito da coisa. A mesma situação vai acontecer com as provas de $4$ linhas, com as de $5$ linhas,... e com as de $n$ linhas, para qualquer número finito $n$ de linhas. Ou seja, todas as linhas de todas as provas da LPC são inocentes, o que demonstra o Lema da Inocência.$\blacklozenge$\footnote{
...
O único passo que ainda falta para completarmos de vez a prova do Teorema da Correção é provar que todas as regras de inferência da LPC são mesmo confiáveis.
Apesar de não ser especialmente difícil, esta é uma prova longa, já que a LPC tem muitas regras de inferência e precisaremos verificar a confiabilidade de cada regra individualmente.
...
Ou seja, a notação  `se  \ ({para todo}  $i<n$, $\Delta_i \entails \meta{S}_i$) \ \ então \ \ $\Delta_n \entails \meta{S}_n$' afirma precisamente a propriedade da confiabilidade que queremos demonstrar que é satisfeita por todas as regras de inferência da LPC.

A ideia é enunciar um lema para cada regra de inferência da LPC e provar que cada uma delas é confiável.
Então, mãos à obra.
...
\noindent\emph{Prova.} Considere uma aplicação qualquer de $\eand$I em uma prova qualquer da LPC.
...
Todos os lemas restantes, que estabelecem a confiabilidade das demais regras da LPC terão, essencialmente, a mesma estrutura que este.
...
Até aqui já demonstramos que todas as regras básicas do sistema formal de provas da LPC são confiáveis.
Vamos agora demonstrar que qualquer regra derivada também é confiável.

\begin{factoidboxe}
	Todas as regras derivadas de nosso sistema formal para a LPC são confiáveis.
\end{factoidboxe}
\noindent\emph{Prova.}
	Suponha que tenhamos usado uma regra derivada para obter alguma sentença, $\meta{A}$, na linha $n$ de alguma prova da LPC, e que todas as linhas anteriores da prova são inocentes.
...
Demonstramos que o sistema formal de provas da LPC que aprendemos neste livro é Correto. Isto significa que qualquer prova formal que fizermos nesse sistema será a prova de um argumento válido de acordo com a semântica das tabelas de verdade.
...
Mas a ideia geral da Correção de um sistema formal é simples e, no caso da LPC, além de simples ela, desde o princípio, pareceu plausível, quase óbvia.

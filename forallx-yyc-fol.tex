...
\end{earg}
Repare que todas as sentenças deste argumento são atômicas, ou seja, não são compostas por sentenças mais simples ligadas por conectivos proposicionais.
Por isso qualquer chave de simbolização para o argumento na LPC seria semelhante a esta:
\begin{ekey}
...
%	\item[\therefore] $R$
%\end{earg}
Essa forma argumentativa proposicional é \emph{inválida}, porém o argumento original em português é claramente válido.
...
Mas há uma valoração na qual `$L$' e `$T$' são verdadeiras e `$R$' é falsa e, portanto, a forma puramente proposicional do argumento não é válida.
Essa valoração, no entanto, não é contemplada por nenhuma situação concebível.
Isso demonstra que há uma assincronia (uma discrepância) entre as situações às quais as sentenças do  argumento original em português podem estar se referindo e as valorações do argumento simbolizado na LPC.

Acontece que não cometemos nenhum erro de simbolização.
Esta é a melhor simbolização na LPC que podemos oferecer a este argumento.
Qualquer outra chave de simbolização aceitável na LPC diferiria desta apenas nas letras sentenciais utilizadas.
O argumento simbolizado continuaria inválido em qualquer uma delas.
Este fato, conforme vimos na Seção \ref{s:ParadoxesOfMaterialConditional}, é um indício de que este argumento está além dos limites da LPC e vai requerer uma nova linguagem, com mais recursos, para que possa ser adequadamente simbolizado.
...
No entanto, todas estas sentenças são atômicas e por isso são todas simbolizadas na LPC por letras sentenciais independentes.
...
A LPO é significativamente mais complexa e sutil que a LPC e, por isso, vamos começar agora a estudá-la lentamente.
...
Como na LPC, utilizaremos chaves de simbolização também na LPO.
...
Então nós utilizamos aqui os recursos da LPC e simbolizamos a sentença inteira por
$$\atom{B}{g} \eand \atom{B}{m}$$
Isso ilustra um ponto importante:
a lógica de primeira ordem possui todos os conectivos da lógica proposicional.
...
\section{Sentenças quantificadas comuns}
...
E lembre-se de que, na LPC, \hbox{$\meta{A} \eif \meta{B}$} é logicamente equivalente a $\enot\meta{A} \eor \meta{B}$.
...
O âmbito de uma quantificação é muito parecido com o âmbito da negação, que estudamos na LPC.
...
Pois, conforme veremos na próxima Parte do livro, assim como na LPC, na LPO também existem apenas dois valores de verdade (o Verdadeiro e o Falso), e toda sentença tem de ter um desses dois valores, não havendo qualquer outra alternativa.
...
A questão, no entanto, é controversa e tem muitos desdobramentos.\footnote{
	Se você estiver interessado no debate pode consultar o artigo `Speaker Reference and Semantic Reference' de Saul Kripke, publicado em French et al (eds.), \emph{Contemporary Perspectives in the Philosophy of Language}, Minneapolis: University of Minnesota Press, 1977, pp.\ 6-27.}
...
Na LPO existem dois tipos de fórmulas: fórmulas em que ocorrem variáveis não quantificadas, chamadas \emph{fórmulas abertas}, e fórmulas em que não existem variáveis não quantificadas, chamadas \emph{fórmulas fechadas}.

Abaixo esses conceitos serão definidos precisamente. 
Para tanto, precisamos definir a noção de termo.
	\factoidbox{
...
Repare que no item 1 do quadro acima consideramos as letras sentenciais (da LPC) como fórmulas atômicas da LPO; ao final de nossa definição veremos que toda sentença LPC também será uma sentença da LPO.
...
As primeiras cláusulas são exatamente idênticas às da LPC.
	\factoidbox{
	\begin{enumerate}
...
Aqui seguimos o caso da LPC, com a observação de que a noção de operador lógico principal corresponde ao acréscimo dos quantificadores à noção de conectivo principal definida no Capítulo \ref{s:TFLSentences}.
	\factoidbox{
...
\section{Fórmulas abertas e fechadas}


Seja o domínio de discurso constituído de pessoas e seja a seguinte chave de simbolização:
\begin{center}
	\begin{ekey}
		\item[\atom{A}{x,y}] \gap{x} ama \gap{y}
		\item[b] Boris
...
Essa fórmula poderia ser tomada como a formalização de uma sentença do tipo

\begin{earg}
\item[(A)] \emph{fulano} ama a si mesmo
\end{earg}

A menos que seja especificado a quem se refere o termo `fulano', (A) não é nem verdadeira nem falsa.
Do mesmo modo, sem a atribuição de uma referência à variável $z$, a fórmula `$\atom{A}{z,z}$'  não pode ser nem verdadeira nem falsa.
Por outro lado, ao colocamos um quantificador existencial, por exemplo, à esquerda de `$\atom{A}{z,z}$', resultando em `$\exists z \atom{A}{z,z}$', a fórmula obtida corresponde a uma asserção que será verdadeira se alguém amar a si próprio.
Da mesma forma, com um quantificador universal obteríamos `$\forall z \atom{A}{z,z}$', que é verdadeira se todos amam a si mesmos.

Vamos dar algumas definições envolvidas nessas considerações:

\factoidbox{
	\begin{itemize}
		\item Uma ocorrência de uma variável \meta{x} é uma \define{ocorrencia ligada} se está dentro do escopo de $\forall \meta{x}$ ou de $\exists \meta{x}$. 	
		\item Qualquer ocorrência de variável que não seja ligada, é uma \define{ocorrencia livre}.
                \item $\meta{A}$ é uma \define{formula aberta} quando contém variáveis livres, isto é, que não estão sob o escopo de um quantificador, caso contrário, $\meta{A}$ é uma \define{formula fechada}, ou uma \define{sentence of FOL}.
	\end{itemize}
	}

Assim, na fórmula:
	$$\forall x(\atom{E}{x} \eor \atom{D}{y}) \eif \exists z(\atom{E}{x} \eif \atom{L}{z,x})$$
o escopo do quantificador universal `$\forall x$' é `$\forall x (\atom{E}{x} \eor \atom{D}{y})$', portanto, a primeira ocorrência de `$x$' é ligada por `$\forall x$'.
...
Resumindo, de modo a ser possível avaliar se certa fórmulas aberta $\meta{A}$ é verdadeira, devemos
\begin{itemize}
	\item ou especificar denotações para todas as ocorrências livres de variáveis $\meta{A}$.
	\item ou quantificar todas as ocorrências livres de variáveis em $\meta{A}$
\end{itemize}
...
Adotaremos na LPO as mesmas convenções sobre o uso de parênteses que adotamos na LPC (veja o Capítulo \ref{s:TFLSentences} e a Seção \ref{s:MoreBracketingConventions}):
\begin{ebullet}

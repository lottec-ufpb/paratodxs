...
%Na LPC, esta ponte entre as sentenças simbolizadas e as situações é feita pelas valorações.
...
As letras sentencias são tratadas exatamente do mesmo modo que na LPC e quando \meta{R} é um predicado $n$-lugares e \mbox{$\meta{a}_1 $, $\meta{a}_{2}$, \dots, $\meta{a}_{n}$} \ são nomes, temos:
...
Considere o seguinte forma argumentativa na LPO:
\begin{earg}
...
Para mostrar que ela é inválida, precisamos encontrar uma interpretação na qual a premissa seja verdadeira, mas a conclusão seja falsa.
...
\end{earg}
	...
	\end{ebullet}}
Uma interpretação que refuta uma proposta, ou seja, que mostra que uma forma argumentativa \emph{não} é válida, ou que uma fórmula \emph{não} é válida (logicamente necessária), ou que um conjunto de sentenças \emph{não} sustenta logicamente uma outra sentença, é chamada de \emph{contrainterpretação} ou \emph{contramodelo}.
...
\problempart
\label{pr.Contingent}
Mostre que cada uma das fórmulas abaixo não é nem uma validade (necessidade lógica) nem uma contradição.
\begin{earg}
...
\problempart
\label{pr.NotEquiv}
Mostre que as fórmulas de cada um dos pares seguintes não são logicamente equivalentes.
\begin{earg}
...
\problempart
Mostre que as fórmulas de cada um dos grupos abaixo são conjuntamente satisfatórias.
\begin{earg}
...
\problempart
Mostre que as formas argumentativas abaixo são inválidas.
\begin{earg}
...
\section{Validades e contradições}
Para mostrar que uma fórmula não é uma validade basta apresentarmos uma única interpretação na qual ela seria falsa.
Mas para mostrar que uma fórmula é uma validade, não é suficiente propor dez, cem, nem mesmo mil interpretações nas quais ela seria verdadeira.
Uma fórmula é uma validade apenas se for verdadeira em \emph{todas} as interpretações; e há infinitas delas.
...
Veja como podemos mostar que a fórmula \mbox{`$\atom{R}{a,a}\eor\enot \atom{R}{a,a}$'} é uma validade:
	\begin{quote}
		\label{allmodels1}
		Qualquer interpretação relevante (ou seja que leve em consideração a relação `$R$' e o nome `$a$') tornará `$\atom{R}{a,a}$' verdadeira ou falsa.
		Então, se pensarmos na totalidade das interpretações relevantes, podemos dividir esta totalidade em dois grupos, o grupo (1), constituído pelas interpretações em que `$\atom{R}{a,a}$' é verdadeira, e o grupo (2), com as interpretações em que  `$\atom{R}{a,a}$' é falsa.
		...
		Como `$\atom{R}{a,a}\eor\enot \atom{R}{a,a}$' é verdadeira em todas as interpretações do grupo (1) e do grupo (2), e qualquer interpretação está em um destes dois grupos, então `$\atom{R}{a,a}\eor\enot \atom{R}{a,a}$' é verdadeira em todas as interpretações e é, portanto, uma validade.
...
Uma outra característica deste nosso argumento metalinguístico é que como a fórmula em questão não contem quantificadores, não precisamos pensar em como interpretar `$a$' e `$R$'; dissemos apenas que qualquer que seja a interpretação,  `$\atom{R}{a,a}$' será verdadeira ou falsa.
Poderíamos ter argumentado de modo similar a respeito de fórmulas da LPC.

Eis aqui um outro exemplo de raciocínio sobre todas as interpretações.
Considere a fórmula `$\forall x(\atom{R}{x,x}\eor\enot \atom{R}{x,x})$'.
Novamente, ela parece uma validade óbvia, mas explicar precisamente por quê, é um grande desafio.
Não podemos dizer simplesmente que `$\atom{R}{x,x} \eor\enot \atom{R}{x,x}$' é verdadeira em todas as interpretações, uma vez que `$\atom{R}{x,x} \eor\enot \atom{R}{x,x}$' não é nem mesmo uma \emph{fórmula fechada} da LPO (lembre-se de que `$x$' é uma variável, não um nome).
...
Há outros casos que também exigem que raciocinemos sobre todas as interpretações. Devemos fazê-lo se quisermos mostrar:
	\begin{ebullet}
	\item que uma fórmula é contraditória; pois isto requer que ela seja falsa em \emph{todas} as interpretações.
	\item que duas fórmulas são logicamente equivalentes; pois isso requer que elas tenham o mesmo valor de verdade em \emph{todas} as interpretações.
	\item que algumas fórmulas são conjuntamente insatisfatórias; pois isso requer que não haja interpretação na qual elas sejam verdadeiras; ou seja, que, em \emph{toda} interpretação, pelo menos uma delas seja falsa.
	\item que uma forma argumentativa é válida; pois isso requer que a conclusão seja verdadeira em \emph{toda} interpretação em que as premissas são verdadeiras.
	\item que certas sentenças sustentam alguma outra, pois isso também requer que a sentença sustentada seja verdadeira em \emph{toda} interpretação em que as sentenças que sustentam são verdadeiras.
	\end{ebullet}
        O problema é que, com as ferramentas que temos no momento, raciocinar sobre todas as interpretações é um sério desafio!
        Vamos ver apenas mais um exemplo.
        Aqui está uma forma argumentativa que é obviamente válida:
	$$\forall x(\atom{H}{x} \eand \atom{J}{x}) \ \therefore \ \forall x\, \atom{H}{x}$$
Afinal de contas, se tudo é $H$ e $J$, então tudo é $H$.
Mas só conseguimos mostrar que esta forma argumentativa é válida se mostrarmos o que deve ser verdadeiro em todas as interpretações nas quais a premissa é verdadeira.
Para mostrar isso temos que raciocinar da seguinte maneira:
	\begin{quote}
		...
		Por isto, a forma é válida!
\end{quote}
        Veja que mesmo para uma forma simples como esta, o raciocínio que demonstra sua validade é um pouco complicado.
        Para argumentos mais longos e complexos, a demonstração de sua validade pode ser extremamente torturante.
...
\textbf{Teste} & \textbf{Sim} & \textbf{Não}\\
 \hline
\cline{1-3}
a fórmula é válida? & todas & uma \\
a fórmula é contraditória? &  todas  & uma \\
as fórmulas são equivalentes? & todas & uma \\
as fórmulas são conjuntamente satisfatórias? & uma & todas \\
A forma argumentativa é válida? & todas & uma \\

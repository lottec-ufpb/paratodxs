...
Dentre estas sentenças, uma é destacada como sendo o alvo do argumento, a \define{conclusion}.
...
As \definepl{sentence} que compõem um argumento são frases declarativas que, em princípio, poderiam ser verdadeiras ou falsas.
Observe que interrogações, interjeições, perguntas, súplicas, imperativos e ordens, dentre outras estruturas linguísticas, não serão parte daquilo que estamos chamando de \define{argument}, pois não faria sentido dizer que são verdadeiras ou falsas.
...
No estudo da validade, portanto, é importante abstrair do conteúdo das sentenças, aquilo que não concerne as expressões lógicas, trazendo à tona assim a forma lógica dos argumentos.
...
No exemplo recém-apresentado, expressamos cada premissa através de uma oração separada, e usamos uma terceira oração para  expressar a conclusão do argumento.
...
Independente desta ordem, porém, o importante aqui é percebermos que um discurso qualquer será um argumento sempre que ele for a expressão de um raciocínio, no qual uma conclusão é sustentada por uma ou mais premissas. 
...
É importante tornar esta noção de ``situação'' mais precisa e investigar em que medida diferentes modos de tornar mais precisa a noção de ``situação'' afetam quais argumentos serão classificados como válidos e quais não serão. 
...
\end{enumerate}
Em quais dos 5 grupos abaixo as sentenças são compatíveis? E em quais elas são incompatíveis?
\begin{earg}
